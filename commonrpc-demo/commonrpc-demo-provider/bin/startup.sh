...
if [ "$ISRUN" != "1" ]; then
    ...
    nohup $JAVA_HOME/bin/java -classpath $CLASSPATH com.commonrpc.demo.ProviderStartup > $BASE/logs/stdout.log 2>&1 &
    echo $!> $BASE/logs/project.pid

    #$JAVA_HOME/bin/java -classpath $CLASSPATH com.commonrpc.demo.ProviderStartup
